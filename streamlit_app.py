...
if upload_file is not None:
    ...
    G = nx.from_pandas_edgelist(
        df_edges, "source", "target", edge_attr="weight", create_using=nx.Graph()
    )
else:
    n_nodes = st.sidebar.slider("Sample network size", 20, 200, 40, 10)
    m_edges = st.sidebar.slider("Edges per new node (Barabási–Albert)", 1, 5, 2)
...
seed_nodes = st.sidebar.multiselect(
    "Early adopters / shock origin", all_nodes, default=seed_default
)
activation_prob = st.sidebar.slider("Activation probability (β)", 0.01, 0.5, 0.15, 0.01)
...
if st.sidebar.button("Run Simulation"):
    if not seed_nodes:
        st.error("Please select at least one seed node to start the diffusion.")
    else:
        with st.spinner("Simulating diffusion…"):
            active, steps = independent_cascade(G, seed_nodes, activation_prob)
            ...
            ax.set_title("Independent Cascade Diffusion Curve")
            st.pyplot(fig)
...
st.sidebar.info(
    ...
    - **Early plateaus** → consider expanding seed set or boosting β via incentives.
    - **Sparse activation** → indicates structural silos or weak ties to bridge.
    """
)
